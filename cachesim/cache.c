...
//	uint8_t block_inside_offset = (addr & 0x3f); //用于记录块内偏移量
	uint8_t block_inside_offset = ((addr & 0x3f) & ~0x3); //用于记录块内偏移量
//	printf("----READ-----||block_inside_offset:0x%x\n",(uint32_t)block_inside_offset);
	uint8_t index = ((addr >> 6) & 0x1); //用于cache组号
//	printf("----READ-----||index:0x%x\n",(uint32_t)index);
	uint16_t tag = ((addr >> 7) & 0x1ffff); //用于记录块群号
//	printf("----READ-----||tag:0x%x\n",(uint32_t)tag);
	mem_block_NO = ((mem_block_NO + tag) << 1)+index;

    for(int i = 0; i < 2; i ++) {
	    if(cache[index*2+i].tag == tag && cache[index*2+i].valid == true) {
////			printf("!!!!!!!!HIT!!!!!!!!\n");
		    hit = true;
			...
			for(int j = 4; j > 0; j --) {
			    result += (cache[index*2+i].block[block_inside_offset+j-1] << (j-1)*8);
	...
	if(hit == false) { //未命中，访问内存
		...
		full = cache[index*2].valid && cache[index*2+1].valid; //先检查对应的cache组是否满了
		if(full == true) {
		    uint8_t random_select = rand() % 2; //满了随机选择一个替换
			if(cache[index*2+random_select].dirty == true) {
				uint16_t old_mem_block_NO = 0;
				old_mem_block_NO = ((old_mem_block_NO + cache[index*2+random_select].tag) << 1)+index; //计算将要被替换的一块所对应的主存块号
				mem_write(old_mem_block_NO, &(cache[index*2+random_select].block[0])); //回写
			}
		    mem_read(mem_block_NO, &(cache[index*2+random_select].block[0]));
			cache[index*2+random_select].tag = tag;
			cache[index*2+random_select].valid = true;
			cache[index*2+random_select].dirty = false; //完成从内存读取替换
			
			for(int j = 4; j > 0; j --) {
			    result += (cache[index*2+random_select].block[block_inside_offset+j-1] << (j-1)*8);
			}
...
////		    printf("!!!!!!!!HIT!!!!!!!!\n");
		    ...
		    switch(wmask) {
			    case 0x0: assert(0); break;
			    case 0xff: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[0], 1); break;
			    case 0xff00: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[1], 1); break;
			    case 0xff0000: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[2], 1); break;
			    case 0xff000000: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[3], 1); break;
			    case 0xffff: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[0], 2); break;
			    case 0xffff00: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[1], 2); break;
			    case 0xffff0000: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[2], 2); break;
			    case 0xffffff: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[0], 3); break;
			    case 0xffffff00: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[1], 3); break;
			    case 0xffffffff: memcpy(&(cache[index*2+i].block[block_inside_offset]), &data_set[0], 4); break;
				default: printf("------Shoul not reach here!!!------wmask:0x%x\n",wmask); break;
			}
			cache[index*2+i].dirty = true;
	...
	if(hit == false) { //未命中，访问内存
		...
		if(full == true) {
		    ...
		    switch(wmask) {
			    case 0x0: assert(0); break;
			    case 0xff: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[0], 1); break;
			    case 0xff00: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[1], 1); break;
			    case 0xff0000: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[2], 1); break;
			    case 0xff000000: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[3], 1); break;
			    case 0xffff: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[0], 2); break;
			    case 0xffff00: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[1], 2); break;
			    case 0xffff0000: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[2], 2); break;
			    case 0xffffff: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[0], 3); break;
			    case 0xffffff00: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[1], 3); break;
			    case 0xffffffff: memcpy(&(cache[index*2+random_select].block[block_inside_offset]), &data_set[0], 4); break;
				default: printf("------Shoul not reach here!!!------wmask:0x%x\n",wmask); break;
			}
			cache[index*2+random_select].dirty = true;
...
//	uint8_t block_inside_offset = (addr & 0x3f); //用于记录块内偏移量
	uint8_t block_inside_offset = ((addr & 0x3f) & ~0x3); //用于记录块内偏移量
//	printf("----READ-----||block_inside_offset:0x%x\n",(uint32_t)block_inside_offset);
	uint8_t index = ((addr >> 6) & 0xf); //用于cache组号
//	printf("----READ-----||index:0x%x\n",(uint32_t)index);
	uint16_t tag = ((addr >> 10) & 0x3ff); //用于记录块群号
//	printf("----READ-----||tag:0x%x\n",(uint32_t)tag);
	mem_block_NO = ((mem_block_NO + tag) << 4)+index;
