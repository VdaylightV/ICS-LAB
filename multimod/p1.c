...
static inline char* multi(char a[], int len_a, char b[], int len_b, char res[], int len_res) {
	   ...
	   // len_a_m+len_b_m = len_a+len_b-1
	   char temp_res[len_a + len_b - 1];
	   for(int i = 0; i < len_b-1; i ++) { //对应于下面一行的每一位

	           for(int i = 0; i < len_a + len_b - 1; i ++) { //对应于上面一行的每一位
	               temp_res[i] = '0';
	           }
	           temp_res[len_a + len_b -2] = '\0'; //由于竖式实际上是将下面的数的每一位去乘上面的数，因此，先把储存乘出来结果的数组清空

	       char bit_b = str_b_m[len_b-1-1-i]; //竖式下面的那一行的每一位进行循环

		   for(int j = 0; j < len_a - 1; j ++) {

		       char bit_a = str_a_m[len_a-1-1-j]; //对下面竖式的每一位进行循环
			   int res = (bit_a - '0') * (bit_b - '0'); //计算两位数相乘的结果
			   
               if(res <= 9) {
			       if(temp_res[len_a+len_b-1-1-i-j] + res <= '9') {
			           temp_res[len_a+len_b-1-1-i-j] += res;
			       }	   //若乘出来的结果加到相应的temp_res位上，不产生进位，那么简单的temp_res位做加法即可
			       else {
			           temp_res[len_a+len_b-1-1-i-j] = res - 10 + temp_res[len_a+len_b-1-1-i-j];
                       temp_res[len_a+len_b-1-1-i-j-1] += 1;
			       ...
			       if(temp_res[len_a+len_b-1-1-i-j] + left_bit <= '9') { //若相乘结果的个位加到temp_res后不产生进位
			           temp_res[len_a+len_b-1-1-i-j] += left_bit;        //那么forward_bit就是进位，放到前一位即可
					   temp_res[len_a+len_b-1-1-i-j-1] += forward_bit;
			       }	   
			       else {                                                  //若相乘结果个位加到temp_res后产生进位 
					   forward_bit += 1;                                   //此时的进位至多为1，forward_bit加1是进位
					   temp_res[len_a+len_b-1-1-i-j] = temp_res[len_a+len_b-1-1-i-j] + left_bit - 10;
					   temp_res[len_a+len_b-1-1-i-j-1] += forward_bit;
			   }
		   }
		   }
		   ...
		   for(int k = 0; k < len_a + len_b-1; k ++) {
		       int bit = temp_res[len_a + len_b-1-1-k] - '0';
			   if(str_res[len_a + len_b-1-1-k] + bit <= '9') {
			       str_res[len_a + len_b-1-1-k] += bit;
			   }
			   
			   else {
			       str_res[len_a + len_b-1-1-k] = str_res[len_a + len_b-1-1-k] + bit - 10;
				   str_res[len_a + len_b-1-1-k-1] += 1;

				   if(str_res[len_a + len_b-1-1-k-1] > '9') {
				       str_res[len_a + len_b-1-1-k-1] -= 10;
					   str_res[len_a + len_b-1-1-k-1-1] += 1;
				   }
				       if(str_res[len_a + len_b-1-1-k-1-1] > '9') {
				           str_res[len_a + len_b-1-1-k-1-1] -= 10;
					       str_res[len_a + len_b-1-1-k-1-1-1] += 1;
				       }
				           if(str_res[len_a + len_b-1-1-k-1-1-1] > '9') {
				               str_res[len_a + len_b-1-1-k-1-1-1] -= 10;
					           str_res[len_a + len_b-1-1-k-1-1-1-1] += 1;
				           }
				               if(str_res[len_a + len_b-1-1-k-1-1-1-1] > '9') {
				                   str_res[len_a + len_b-1-1-k-1-1-1-1] -= 10;
					               str_res[len_a + len_b-1-1-k-1-1-1-1-1] += 1;
				               }
				                   if(str_res[len_a + len_b-1-1-k-1-1-1-1-1] > '9') {
				                       str_res[len_a + len_b-1-1-k-1-1-1-1-1] -= 10;
					                   str_res[len_a + len_b-1-1-k-1-1-1-1-1-1] += 1;
